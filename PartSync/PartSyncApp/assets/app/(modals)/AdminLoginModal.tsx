...
export default function AdminLoginModal({ visible, onClose, onSuccess }: Props) {
  ...
  const handleLogin = async () => {
    ...
    setError("");
    try {
      const res = await axios.post("http://10.102.232.54:5000/admin/login", { email, password });
  ...
  const handleOTPVerify = async () => {
    ...
    setError("");
    try {
      const res = await axios.post("http://10.102.232.54:5000/admin/verify-otp", { email, otp });
