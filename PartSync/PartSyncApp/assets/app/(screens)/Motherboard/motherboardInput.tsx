...
export default function MotherboardInputScreen() {
  ...
  useEffect(() => {
    fetch('http://10.102.232.54:5000/api/hardware/cpu').then(res => res.json()).then(data => { setCpus(data); setFilteredCpus(data); }).finally(() => setLoadingCpu(false));
    fetch('http://10.102.232.54:5000/api/hardware/gpu').then(res => res.json()).then(data => { setGpus(data); setFilteredGpus(data); }).finally(() => setLoadingGpu(false));
    fetch('http://10.102.232.54:5000/api/hardware/ram').then(res => res.json()).then(data => { setRams(data); setFilteredRams(data); }).finally(() => setLoadingRam(false));
  }, []);
