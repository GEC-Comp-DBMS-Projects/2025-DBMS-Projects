...
const MotherboardRecommendationScreen = () => {
  ...
  const shareMobo = async (mobo: any) => {
    const baseUrl = 'https://myapp.com/motherboard';
    const params = encodeURIComponent(JSON.stringify(mobo));
    const shareUrl = `${baseUrl}?data=${params}`;
    try {
      await Share.share({
        message: `Check out this Motherboard: ${mobo.name}\n${shareUrl}`,
      });
    } catch (err) {
  ...
  const shareAllRecommendations = async () => {
    ...
    try {
      await Share.share({
        message: `Check out my Motherboard recommendations:\n${shareUrl}`,
      });
    } catch (err) {
          ...
          <View style={[styles.modalContainer, { backgroundColor: theme.cardBackground }]}>
            ...
            {selectedMobo && (
              <ScrollView contentContainerStyle={styles.modalContent}>
                <Image
                  source={{ uri: selectedMobo.image || 'https://via.placeholder.com/400x200.png?text=No+Image' }}
                ...
                <TouchableOpacity
                  ...
                  onPress={() => shareMobo(selectedMobo)}
                >
