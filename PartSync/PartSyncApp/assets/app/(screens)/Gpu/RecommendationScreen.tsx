...
const RecommendationScreen = () => {
  ...
  const shareGPU = async (gpu: any) => {
    const shareUrl = `https://myapp.com/gpu?data=${encodeURIComponent(JSON.stringify(gpu))}`;
    try {
      await Share.share({ message: `Check out this GPU: ${gpu.name}\n${shareUrl}` });
    } catch (err) {
      console.error('❌ Error sharing GPU:', err);
    }
  };

  const shareAllRecommendations = async () => {
    const shareUrl = `https://myapp.com/gpu?data=${encodeURIComponent(JSON.stringify(data))}`;
    try {
      await Share.share({ message: `Check out my GPU recommendations:\n${shareUrl}` });
    } catch (err) {
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      <Text style={[styles.header, { color: theme.textPrimary }]}>Top GPU Recommendations</Text>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        {data.map((gpu, index) => (
          ...
          >
            <Image source={{ uri: gpu.image }} style={styles.cardImage} resizeMode="contain" />

            <View style={styles.cardInfo}>
              <Text style={[styles.cardName, { color: theme.textPrimary }]}>{gpu.name}</Text>
              <Text style={[styles.cardPrice, { color: theme.textPrimary }]}>₹{gpu.price}</Text>
      ...
      <Modal
        ...
        animationType="slide"
        transparent
        onRequestClose={() => setSelectedGPU(null)}
      >
        <View style={styles.modalBackdrop}>
          <View style={[styles.modalContainer, { backgroundColor: theme.cardBackground }]}>
            <TouchableOpacity style={styles.closeIcon} onPress={() => setSelectedGPU(null)}>
              <Ionicons name="close-circle" size={30} color={theme.icon} />
            </TouchableOpacity>

            {selectedGPU && (
              <ScrollView contentContainerStyle={styles.modalContent}>
                <Image source={{ uri: selectedGPU.image }} style={styles.modalImage} resizeMode="contain" />
                <Text style={[styles.modalName, { color: theme.textPrimary }]}>{selectedGPU.name}</Text>

                <View style={[styles.infoBox, { backgroundColor: theme.background }]}>
                  <Text style={[styles.modalDetail, { color: theme.textSecondary }]}>💾 VRAM: {selectedGPU.vram} GB</Text>
                  <Text style={[styles.modalDetail, { color: theme.textSecondary }]}>🔥 TDP: {selectedGPU.tdp} W</Text>
                  <Text style={[styles.modalDetail, { color: theme.textSecondary }]}>⚡ Performance: {selectedGPU.score}</Text>
                  <Text style={[styles.modalDetail, { color: theme.textSecondary }]}>💰 Price: ₹{selectedGPU.price}</Text>
                </View>

                <View style={styles.tags}>
                  {selectedGPU.tags?.map((tag: string, i: number) => (
                    <View key={i} style={[styles.tag, { backgroundColor: getBadgeColor(tag) }]}>
                      <Text style={styles.tagText}>{tag}</Text>
                    </View>
                  ))}
