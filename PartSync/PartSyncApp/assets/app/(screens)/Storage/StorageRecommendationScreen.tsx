...
const StorageRecommendationScreen = () => {
  ...
  const getBadgeColor = (label: string) => {
    ...
    if (label.includes("Balanced")) return "#FFC107";
    return theme.icon;
  };

  const shareDrive = async (drive: any) => {
    const baseUrl = 'https://myapp.com/storage';
    const params = encodeURIComponent(JSON.stringify(drive));
    const shareUrl = `${baseUrl}?data=${params}`;
    try {
      await Share.share({
        message: `Check out this storage: ${drive.name}\n${shareUrl}`,
      });
    } catch (err) {
  ...
  const shareAllRecommendations = async () => {
    ...
    try {
      await Share.share({
        message: `Check out my storage recommendations:\n${shareUrl}`,
      });
    } catch (err) {
  ...
  const labelMap = ["🥇 Best Overall", "⚡ High Performance", "💰 Budget Option"];
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        {data.map((drive, index) => (
          ...
          >
            <Image
              source={{ uri: drive.image || 'https://via.placeholder.com/400x200.png?text=No+Image' }}
          ...
          <View style={[styles.modalContainer, { backgroundColor: theme.cardBackground }]}>
            ...
            {selectedDrive && (
              <ScrollView contentContainerStyle={styles.modalContent}>
                <Image
                  source={{ uri: selectedDrive.image || 'https://via.placeholder.com/400x200.png?text=No+Image' }}
