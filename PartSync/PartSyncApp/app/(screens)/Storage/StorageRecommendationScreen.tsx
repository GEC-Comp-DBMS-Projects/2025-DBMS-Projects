...
const StorageRecommendationScreen = () => {
  ...
  const getBadgeColor = (label: string) => {
    if (label.includes('Fast')) return '#4CAF50';
    if (label.includes('Large Capacity')) return '#2196F3';
  ...
  const shareDrive = async (drive: any) => {
    const payload = {
      title: `Storage Recommendation: ${drive.name}`,
    ...
    try {
      await Share.share({
        message: `Check out this storage drive I found on PartSync:\n${drive.name}\n${url}`,
      });
    } catch (err) {
      console.error('❌ Error sharing storage:', err);
    }
  };
  ...
  const shareAllRecommendations = async () => {
    ...
    const components = data.reduce((acc, drive, index) => {
      acc[`storage_${index + 1}`] = drive;
      return acc;
    }, {} as any);

    const payload = {
      title: 'My Storage Recommendations',
      description: 'Shared from PartSync',
      components,
    ...
    try {
      await Share.share({
        message: `Here are my storage recommendations from PartSync:\n${url}`,
      });
    } catch (err) {
      console.error('❌ Error sharing all storage recommendations:', err);
    }
  };
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        {data.map((drive, index) => (
          ...
          >
            <Image
              source={{ uri: drive.image || 'https://placehold.co/400x200?text=No+Image' }}
              style={styles.cardImage}
              resizeMode="contain"
            />

            <View style={styles.cardInfo}>
              <Text style={[styles.cardName, { color: theme.textPrimary }]}>{drive.name}</Text>
              <Text style={[styles.cardPrice, { color: theme.textPrimary }]}>₹{drive.price}</Text>

              <View style={styles.tags}>
                {drive.tags?.map((tag: string, i: number) => (
          ...
          <View style={[styles.modalContainer, { backgroundColor: theme.cardBackground }]}>
            ...
            {selectedDrive && (
              <ScrollView contentContainerStyle={styles.modalContent}>
                <Image
                  source={{ uri: selectedDrive.image || 'https://placehold.co/400x200?text=No+Image' }}
                ...
                <View style={[styles.infoBox, { backgroundColor: theme.background }]}>
                  ...
                  <Text style={[styles.modalDetail, { color: theme.textSecondary }]}>
                    💾 Capacity: {selectedDrive.capacity} GB
                  </Text>
                  <Text style={[styles.modalDetail, { color: theme.textSecondary }]}>
                    ⚡ Read Speed: {selectedDrive.read_speed} MB/s
                  </Text>
                  <Text style={[styles.modalDetail, { color: theme.textSecondary }]}>
                    ⚡ Write Speed: {selectedDrive.write_speed} MB/s
                  </Text>
                  <Text style={[styles.modalDetail, { color: theme.textSecondary }]}>
                    💰 Price: ₹{selectedDrive.price}
                  </Text>
                </View>

                <View style={styles.tags}>
                  {selectedDrive.tags?.map((tag: string, i: number) => (
                    <View key={i} style={[styles.tag, { backgroundColor: getBadgeColor(tag) }]}>
                      <Text style={styles.tagText}>{tag}</Text>
                    </View>
                  ))}
                ...
                <TouchableOpacity
                  ...
                  onPress={() => shareDrive(selectedDrive)}
                >
...
export default StorageRecommendationScreen;
