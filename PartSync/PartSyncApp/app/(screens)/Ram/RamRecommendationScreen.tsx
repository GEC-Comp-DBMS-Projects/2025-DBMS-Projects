...
const RamRecommendationScreen = () => {
  ...
  const shareRam = async (ram: any) => {
    const payload = {
      title: `RAM Recommendation: ${ram.name}`,
    ...
    try {
      await Share.share({
        message: `Check out this RAM I found on PartSync:\n${ram.name}\n${url}`,
      });
    } catch (err) {
  ...
  const shareAllRecommendations = async () => {
    ...
    const components = data.reduce((acc, ram, index) => {
      acc[`ram_${index + 1}`] = ram;
      return acc;
    }, {} as any);

    const payload = {
      title: 'My RAM Recommendations',
      description: 'Shared from PartSync',
      components,
    ...
    try {
      await Share.share({
        message: `Here are my RAM recommendations from PartSync:\n${url}`,
      });
    } catch (err) {
        ...
        {data.map((ram, index) => (
          ...
          >
            ...
            <View style={styles.cardInfo}>
              <Text style={[styles.cardName, { color: theme.textPrimary }]}>{ram.name}</Text>
              <Text style={[styles.cardPrice, { color: theme.textPrimary }]}>₹{ram.price}</Text>
