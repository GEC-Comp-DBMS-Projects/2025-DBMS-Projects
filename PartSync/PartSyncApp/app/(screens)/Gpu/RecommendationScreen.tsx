...
const RecommendationScreen = () => {
  ...
  const shareGPU = async (gpu: any) => {
    const payload = {
      title: `GPU Recommendation: ${gpu.name}`,
    ...
    try {
      await Share.share({
        message: `Check out this GPU I found on PartSync:\n${gpu.name}\n${url}`,
      });
    } catch (err) {
  ...
  const shareAllRecommendations = async () => {
    ...
    const components = data.reduce((acc, gpu, index) => {
      acc[`gpu_${index + 1}`] = gpu;
      return acc;
    }, {} as any);

    const payload = {
      title: 'My GPU Recommendations',
      description: 'Shared from PartSync',
      components,
    ...
    try {
      await Share.share({
        message: `Here are my GPU recommendations from PartSync:\n${url}`,
      });
    } catch (err) {
